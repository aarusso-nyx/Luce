...
- `clang-format --dry-run --Werror` on the Wave E pure helper headers
...
them with the evidence state. If the manifest points at the parent commit and
the current commit changes only those evidence files, CI accepts that as the
