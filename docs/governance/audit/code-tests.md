...
- HTTP contract basics: public health/version, auth gating for protected routes, selected `/api/info` fields, `/api/state` + `/api/ota` payloads, `/api/ota/check` source precedence, OTA manual/invalid-url/periodic scenarios, selected LED route behaviors, captive portal assets + SPA fallback routes, selected 405 responses.
...
- MQTT: deterministic unsupported-topic response, LED readback topic, control-path effects (`relays/*`, `sensor/threshold`), supported-vs-unsupported `config/*` behavior, `config/http/token` reboot-persistence, and reconnect outage/recovery behavior.
