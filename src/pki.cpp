...
const char* role_prefix(Role role) {
...
RoleMaterial& material(Role role) {
  return g_roles[role_index(role)];
}
...
bool validate_pair(Role role,
  ...
  rc = mbedtls_pk_parse_key(&key, reinterpret_cast<const unsigned char*>(key_pem), std::strlen(key_pem) + 1, nullptr, 0);
  if (rc != 0) {
...
void derive_state(Role role) {
  ...
  if (m.cert_pem[0] == '\0') {
    m.last_error = "none";
    set_role_state(role, m.staged_present ? State::kCertStaged : State::kCsrReady,
                   m.staged_present ? "cert_staged" : "csr_ready");
    return;
  }
  if (validate_pair(role, m.key_pem, m.cert_pem, m.subject, sizeof(m.subject), m.issuer, sizeof(m.issuer),
                    m.fingerprint, sizeof(m.fingerprint))) {
    m.last_error = "none";
...
esp_err_t load_role(Role role) {
  ...
  char cert[48] = {};
  char staged_key[56] = {};
  char staged[kCertPemBufferSize] = {};
  ...
  const auto key_status = luce::nvs::read_string_status(nvs.raw(), key, m.key_pem, sizeof(m.key_pem), "");
  const auto cert_status = luce::nvs::read_string_status(nvs.raw(), cert, m.cert_pem, sizeof(m.cert_pem), "");
  const auto staged_status = luce::nvs::read_string_status(nvs.raw(), staged_key, staged, sizeof(staged), "");
  m.staged_present = (staged_status == luce::nvs::StringReadStatus::kOk && staged[0] != '\0');
...
esp_err_t ensure_key(Role role) {
  ...
  rc = mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_SECP256R1, mbedtls_pk_ec(key), mbedtls_ctr_drbg_random, &ctr_drbg);
  if (rc != 0) {
    set_error(role, "keygen_ec_failed");
    goto done;
  }
  std::memset(m.key_pem, 0, sizeof(m.key_pem));
  rc = mbedtls_pk_write_key_pem(&key, reinterpret_cast<unsigned char*>(m.key_pem), sizeof(m.key_pem));
  if (rc != 0) {
    set_error(role, "keygen_pem_failed");
    goto done;
  }

  if (nvs_write_str(key_pem_name, m.key_pem) != ESP_OK || nvs_write_str(key_alg_name, kKeyAlgEcP256) != ESP_OK) {
  ...
  m.fingerprint[0] = '\0';
  m.last_error = "none";
  set_role_state(role, State::kCsrReady, "key_generated");
  ESP_LOGI(kTag, "%s keygen generated local %s private key; export CSR next", role_name(role), kKeyAlgEcP256);
...
esp_err_t export_csr(Role role, char* out, std::size_t out_size) {
  ...
  rc = mbedtls_pk_parse_key(&key, reinterpret_cast<const unsigned char*>(m.key_pem), std::strlen(m.key_pem) + 1, nullptr, 0);
  if (rc != 0) {
...
esp_err_t stage_cert_begin(Role role) {
  ...
  (void)load_role(role);
  set_role_state(role, material(role).key_pem[0] != '\0' ? State::kCertStaged : State::kEmpty, "stage_begin");
  return ESP_OK;
}
...
esp_err_t stage_cert_append(Role role, const char* line) {
  ...
  auto nvs = luce::nvs::Handle::Open(kPkiNs, NVS_READONLY);
  if (nvs.ok()) {
    (void)nvs.read_string(staged_key, staged, sizeof(staged), "");
  }
  ...
  ESP_RETURN_ON_ERROR(nvs_write_str(staged_key, staged), kTag, "stage cert append");
  (void)load_role(role);
  ESP_LOGI(kTag, "%s cert.append ok bytes=%u", role_name(role), static_cast<unsigned>(std::strlen(staged)));
  return ESP_OK;
}
...
esp_err_t stage_cert_commit(Role role) {
  ...
  if (!nvs.read_string(staged_key, staged, sizeof(staged), "") || staged[0] == '\0') {
  ...
  char issuer[kSubjectBufferSize] = {};
  char fingerprint[kFingerprintBufferSize] = {};
  if (!validate_pair(role, m.key_pem, staged, subject, sizeof(subject), issuer, sizeof(issuer), fingerprint, sizeof(fingerprint))) {
    return ESP_ERR_INVALID_ARG;
  }
  ESP_RETURN_ON_ERROR(nvs_write_str(cert_key, staged), kTag, "promote cert");
  (void)nvs_erase(staged_key);
  std::snprintf(m.cert_pem, sizeof(m.cert_pem), "%s", staged);
  ...
  ESP_LOGI(kTag, "%s cert.commit ok fingerprint=%s subject='%s'", role_name(role), m.fingerprint, m.subject);
  return ESP_OK;
}
